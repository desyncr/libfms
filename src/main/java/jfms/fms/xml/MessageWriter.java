...
public class MessageWriter {
	...
	public String writeXml(Message msg) {
		if (!Validator.requiredMessageFieldsPresent(msg)) {
			LOG.log(Level.WARNING, "skipping message: required fields missing");
			return null;
		}
		...
		try {
			...
			xtw.writeCharacters(Sanitizer.sanitizeSubject(msg.getSubject()));
			xtw.writeEndElement();
			...
			xtw.writeCData(msg.getBody().replaceAll("]]>", "]]]]><![CDATA[>"));
			xtw.writeEndElement();
		...
		final String messageXml = strWriter.toString();
		final byte[] messageBytes = messageXml.getBytes(StandardCharsets.UTF_8);
		if (messageBytes.length > Constants.MAX_REQUEST_SIZE) {
			LOG.log(Level.WARNING, "maximum message size exceeded");
			return null;
		}
		return messageXml;
	}
