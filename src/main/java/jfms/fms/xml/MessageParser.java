		...
		while (reader.hasNext()) {
			if (reader.next() == XMLStreamConstants.START_ELEMENT) {
				if (reader.getLocalName().equals("Message")) {
					...
					LOG.log(Level.INFO, "Message element not found");
				}
				break;
			}
		}
			...
			case XMLStreamConstants.START_ELEMENT:
				...
				if (level == 2) {
					...
					case "Time":
						...
						break;
					case "Subject":
						msg.setSubject(Sanitizer.sanitizeSubject(
									reader.getElementText()));
						level--;
						break;
					case "MessageID":
			...
			case XMLStreamConstants.START_ELEMENT:
				...
				if (level == 2 && reader.getLocalName().equals("Board")) {
					final String sanitized = Sanitizer.sanitizeBoard(
							reader.getElementText());
					if (!sanitized.isEmpty()) {
						if (boards.contains(sanitized)) {
							LOG.log(Level.INFO, "skipping duplicate board");
						} else {
